...
void popnetMessageInsert(long s1, long s2, long d1, long d2, long long int sim_cycle, long size, counter_t msgNo, md_addr_t addr, int operation, int vc)
{
	if(addr < 0)
	    addr = addr * -1;
//	if(msgNo == 7)
		//cout << "New message inserted" << sim_cycle << " " << src_x << " " << src_y << " " << src_z << " " << dest_x << " " << dest_y << " " << dest_z << " " << msgNo << endl;
...
#ifdef MULTI_VC
	if(opt == -1)
		return sim_foundation::wsf().router(network_addr).suggestVC();
	return sim_foundation::wsf().router(network_addr).isBufferFull(0, opt) ? -1:opt;
#else
	return sim_foundation::wsf().router(network_addr).isBufferFull(0, 0);
#endif
}
...
void delay_report(FILE *fd)
{
	...
	fprintf(fd,"total finished:       %d\n",tot_f_t);
	fprintf(fd,"total delay   :       %g\n",total_delay);
...
void power_report(FILE *fd)
{
    ...
    double total_electrical_power = 0;
	...
	for(int j = 0; j < sim_foundation::wsf().ary_size(); j++)
	{
	    ...
	    total_electrical_power += sim_foundation::wsf().router(sor_addr_t).power_report(fd, sim_foundation::wsf().ary_size()*(sim_foundation::wsf().ary_size()-2));
	    total_optical_power += sim_foundation::wsf().ps(sor_addr_t).power_report(fd, sor_addr_t);
	}
    }
    fprintf(fd,"Total Electrical Network power: %g\n", total_electrical_power);
    fprintf(fd,"Total Optical Network power: %g\n", total_optical_power);
}
