...
class sim_foundation {
	...
	private:
		...
		vector<sim_ps_template> ps_network_;
		long ary_size_;
		long cube_size_;
	...
	public:
		...
		const vector<sim_router_template> & inter_network() const {return inter_network_;}
		
		vector<sim_ps_template> & ps_network() {return ps_network_;}
		const vector<sim_ps_template> & ps_network() const {return ps_network_;}		

		sim_router_template & router(const add_type& a);
		const sim_router_template & router(const add_type& a) const;

		sim_ps_template & ps(const add_type& a);
		const sim_ps_template & ps(const add_type& a) const;
		...
		void receive_VIP_message(mess_event mesg);
		void receive_OPT_message(mess_event mesg);
		void receive_TD_message(mess_event mesg);
