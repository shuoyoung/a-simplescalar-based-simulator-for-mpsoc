...
class power_template {
	...
	double power_buffer_report(int telem);
	double power_link_report(int telem);
	double power_crossbar_report(int telem);
	double power_arbiter_report(int telem);
	double power_report(int id, FILE *fd, int telem);
};
//***************************************************************************//
...
class sim_router_template {
	...
	private:
		...
		//long counter_;
		vector<vector<long> >counter_;
        map<long, long> accept_flag_;

	public:
		vector<long> & address() {return address_;}
		const vector<long> & address() const {return address_;}

        map<long,long> & accept_flag() {return accept_flag_;}
        const map<long,long> & accept_flag() const {return accept_flag_;}
		...
		int accept_flit(time_type a, const flit_template & b);

		int isBufferFull(long a, long b) {return counter_[a][b] <= 0;}
		long suggestVC();
		void counter_inc(long a, long b) {counter_[a][b]++;}
		void counter_dec(long a, long b) {counter_[a][b]--;}
		double power_buffer_report(int telem) { return power_module_.power_buffer_report(telem);}
		double power_crossbar_report(int telem) { return power_module_.power_crossbar_report(telem);}
		double power_arbiter_report(int telem) { return power_module_.power_arbiter_report(telem);}
		double power_link_report(int telem) { return power_module_.power_link_report(telem);}
		double power_report(FILE *fd, int telem) {return power_module_.power_report(ary_size_*address_[0]+address_[1], fd, telem);}
		...
		void XYZ_algorithm(const add_type & des_t, const add_type & sor_t, 
				long s_ph, long s_vc);
		void CMESH_XYZ_algorithm(const add_type & des_t, const add_type & sor_t, 
				long s_ph, long s_vc);
		void AFN_E_algorithm(const add_type & des_t, const add_type & sor_t, 
				long s_ph, long s_vc);
