...
/****************************************************************
*	wxh 2012-06-19
*	These variables are added by wxh to support 3D topologies
*	It might be confusing why we have mesh_size and also DIM_X,
*	DIM_Y, and DIM_Z. mesh_size is just a variable s.t. 
*	mesh_size*mesh_size >= total nodes. We use a 2D to 3D ID mapping
*	to keep the original code unchanged as possible.
****************************************************************/
int DIM_X, DIM_Y, DIM_Z;
int C_DIM_X, C_DIM_Y;
int AFNCubeSize, AFNCubeNum;

char * TopoString;
enum TOPO 
{ TwoD_MESH, TwoD_TORUS, TwoD_FT, ThreeD_MESH, ThreeD_CMESH, ThreeD_CTORUS, AFN_O, AFN_E, ThreeD_MECS};
int Topo;
...
char * TracePrintString;
enum TRACEPRINT
{ TRACE_P = 1};
int TracePrintScheme;
...
sim_reg_options (struct opt_odb_t *odb)
{
   ...
   *****************************************************/
  opt_reg_int(odb, "-cache:MPSOC_contexts", "the number of nodes support", &MPSOC_contexts, /* default */ 64,
	      /* print */ TRUE, /* format */ NULL); 
  opt_reg_string (odb, "-redir:EM", "External Memory Read Directory", &EM_dir, /* default */ "none",	/* print */ TRUE, /* format */ NULL);
  opt_reg_string (odb, "-redir:SIM_time", "End sim_time Directory", &SIM_time, /* default */ "none",	/* print */ TRUE, /* format */ NULL);
  opt_reg_string (odb, "-redir:SIM_visited", "real time SIM_visited vertex Directory", &SIM_visited, /* default */ "none",	/* print */ TRUE, /* format */ NULL);
//--------------------------------------------
/****************************************************************
*	wxh 2012-06-19
*	read the parameters
****************************************************************/
        opt_reg_int (odb, "-network:DIM_X", "X dimension size of the network", &DIM_X, /* default */ 4,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_int (odb, "-network:DIM_Y", "Y dimension size of the network", &DIM_Y, /* default */ 4,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_int (odb, "-network:DIM_Z", "Z dimension size of the network", &DIM_Z, /* default */ 4,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_int (odb, "-network:C_DIM_X", "X dimension size of the cluster", &C_DIM_X, /* default */ 2,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_int (odb, "-network:C_DIM_Y", "Y dimension size of the cluster", &C_DIM_Y, /* default */ 2,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_int (odb, "-network:AFNCubeSize", "Cube size of the AFN network", &AFNCubeSize, /* default */ 4,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_int (odb, "-network:AFNCubeNum", "Cube number of the AFN network", &AFNCubeNum, /* default */ 4,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_string (odb, "-network:Topo", "Topologies {2D_MESH| 2D_TORUS| 2D_FT| 3D_MESH|3D_CMESH|3D_CTORUS| AFN_O| AFN_E| 3D_MECS}", &TopoString, /* default */ "3D_MESH",
			/* print */ TRUE, /* format */ NULL);

	opt_reg_string (odb, "-network:Bypass", "Bypass schemes{EVC| VIP| NONE}", &BypassString, /* default */ "NONE",
			/* print */ TRUE, /* format */ NULL);
	opt_reg_string (odb, "-network:Trace", "Need Trace?{TRACE| NONE}", &TracePrintString, /* default */ "NONE",
			/* print */ TRUE, /* format */ NULL);
//--------------------------------------------
	opt_reg_header (odb,
...
/****************************************************************
*	wxh 2012-06-19
*	added by wxh, to initialize the parameters
****************************************************************/
	
	if (!mystricmp (TopoString, "2D_MESH"))
	{
		Topo = TwoD_MESH;
	}
	else if  (!mystricmp (TopoString, "2D_TORUS"))
	{
		Topo = TwoD_TORUS;
	}
	else if  (!mystricmp (TopoString, "2D_FT"))
	{
		Topo = TwoD_FT;
	}
	else if  (!mystricmp (TopoString, "3D_MESH"))
	{
		Topo = ThreeD_MESH;
	}
	else if  (!mystricmp (TopoString, "3D_CMESH"))
	{
		Topo = ThreeD_CMESH;
	}
		else if  (!mystricmp (TopoString, "3D_CTORUS"))
	{
		Topo = ThreeD_CTORUS;
	}
	else if  (!mystricmp (TopoString, "AFN_O"))
	{
		Topo = AFN_O;
	}
	else if  (!mystricmp (TopoString, "AFN_E"))
	{
		Topo = AFN_E;
	}
	else if  (!mystricmp (TopoString, "3D_MECS"))
	{
		Topo = ThreeD_MECS;
	}
	else 
	...
	if (!mystricmp (BypassString, "EVC"))
	{
		BypassScheme = EVC;
	}
	else if  (!mystricmp (BypassString, "VIP"))
	{
		BypassScheme = VIP;
	}
	else if  (!mystricmp (BypassString, "NONE"))
	{
		BypassScheme = NONE;
	}

	if (!mystricmp (TracePrintString, "TRACE"))
	{
		TracePrintScheme = TRACE_P;
	}
	else if  (!mystricmp (TracePrintString, "NONE"))
	{
		TracePrintScheme = NONE;
	}
...
sim_main (void)
{
	...
	/*--------2013-4-22-yangshuo-external memory by a big array------------*/
#ifdef EM  
     	{
	  FILE *fp;
	  long curpos, length;
	  extern char * EMemory;
	  extern int EMint_num,EMchar_num;
	  extern int EMint[EM];
	  
	   if((fp = fopen(EM_dir,"r")) == NULL)
	     //  if((fp = fopen("/home/w/CMP_YS/program/GraphTran_single/EM.txt","r+")) == NULL)
	    {
	      printf("Can't open EM.txt!!!\n");
	      i = getchar();
	    }
	    ...
	    {  
	      ...
	      EMemory = (char*)malloc(sizeof(char)*(length-1));
	      EMchar_num = length - 1;
	      for(i = 0; i < EMchar_num; i++)
		EMemory[i] = fgetc(fp);
	      EMint_num = Change(EMint, EMemory);*/
	      for(length = 0 + 25000000; !feof(fp); length++)//read graph info to EMint at EMint[25000000]
		{
		  fscanf(fp, "%d", &EMint[length]);
		}
	    }
	  fclose(fp);
	}
	...
	for (;;)
	{
#ifdef EM
	  {
	    ...
	    /*if (i_sim == 200)
	      {FILE *fp_o;
		i_sim = 0;
		fp_o = fopen(SIM_visited, "w");
		for (i = 90000000; i < 90002000; i++){
        ...
        */
	    int a;
	    FILE *fp;
	    extern char * sim_time;
	    for (i = 0; i < numcontexts; i++)
	    {
	      a = EMint[EM - numcontexts];
	      if (a == 0)
		break;
	    }
	    // printf("queue_remain now is %d\n",queue_remain );
	    //fflush(stdout);
	  if (a == 1)
	     { 
	    
	       printf("0 - queue_remain is %d \n",queue_remain);
	      fflush(stdout);
	       if (queue_remain <= 1)
			//if (*wxh_q_len<=1)
		{
		  printf("1- queue_remain is %d \n",queue_remain);
		  printf("Work in all the cores is done! @%llu\n", sim_cycle);
		  fflush(stdout);
		  fp = fopen(SIM_time, "w");
		  fprintf(fp, "All the programs finished @%llu!\n", sim_cycle);
		  fclose(fp);
	      
		  break;//break big cycle and exit
		}
	    }
	  }
#endif
...
#define PRINT_STATISTICS 0
	...
	//--external memory related--2013-4-23--yangshuo
#ifdef EM
	//	free(EMemory);
#endif
	//------------------
