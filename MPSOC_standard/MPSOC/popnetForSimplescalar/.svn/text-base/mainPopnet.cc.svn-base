...
char *SIM_package_send;//yangshuo-2013-7-3
char *SIM_trace;
int graph_info_valid;
...
void popnet_options(struct opt_odb_t *odb)
{
  /*yangshuo-7-3 print link and package src and dst*/
  opt_reg_string (odb, "-redir:SIM_package_send", "print link trace", &SIM_package_send, /* default */"", 
		    /* print */ TRUE, /* format */ NULL);

    opt_reg_string (odb, "-redir:SIM_trace", "print link trace", &SIM_trace, /* default */"", 
		    /* print */ TRUE, /* format */ NULL);
    opt_reg_flag (odb, "-graph:graph_info_print", "control graphinfo print or not", &graph_info_valid, /* default */ FALSE,
			/* print */ TRUE, /* format */ NULL);
...
void popnetMessageInsert(long s1, long s2, long d1, long d2, long long int sim_cycle, long size, counter_t msgNo, md_addr_t addr, int operation, int vc)
{
	...
	{//yangshuo-2013-7-3
	  FILE *fp_o;
	  fp_o = fopen (SIM_package_send, "a");
	  fprintf(fp_o, "%ld %ld %llu\n", s1*8+s2-8, d1*8+d2-8, sim_cycle);
	  fclose(fp_o);
	}
	messageQueued++;
}
