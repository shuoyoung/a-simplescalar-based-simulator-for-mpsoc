...
//yangshuo-2013-7-3
extern char *SIM_trace;
extern long long signed sim_cycle;
extern int graph_info_valid;
//***************************************************************************/
...
void sim_router_template::routing_decision()
{
	...
	for(long i = 1; i < physic_ports_; i++) {
		for(long j = 0; j < vc_number_; j++) {
			...
			//yangshuo-2013-7-3
			int X_router, Y_router;
			X_router = address_[0];
			Y_router = address_[1];
			switch (i){
			case 1:
			  X_router = address_[0] - 1;
			  break;
			case 2:
			  X_router = address_[0] + 1;
			  break;
			case 3:
			  Y_router = address_[1] - 1;
			  break;
			case 4:
			  Y_router = address_[1] + 1;
			  break;
			...
			if(input_module_.state(i, j) == ROUTING_) {
			  ...
			  {//yangshuo-2013-7-3
			    FILE *fp_o;
			    fp_o = fopen (SIM_trace, "a");
			    fprintf(fp_o, "%ld %ld %llu\n", X_router*8+Y_router, address_[0]*8+address_[1], sim_cycle);
			    fclose(fp_o);
			  }
				flit_t = input_module_.get_flit(i, j);
				Sassert(flit_t.type() == HEADER_);
