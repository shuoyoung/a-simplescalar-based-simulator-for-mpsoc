...
int dir_fifo_enqueue(struct DIRECTORY_EVENT *event, int type)
{
	...
	else
	{
		...
		//-----------------------------------------DH--2013-4-7
		if((event->operation == CLAIRE_WRITE)||(event->operation == CLAIRE_READ))
		  {
...
void dir_fifo_dequeue()
{
	...
	/* L2 and dir queue */
	for(i = 0; i < MPSOC_contexts+mesh_size*2; i++)/*yangshuo-2013-5-10*/
	{
		// deleted by wxh
	...
	switch(event->operation)
	{
        ...
        case CLAIRE_READ:
	  {
	    ...
	    struct DIRECTORY_EVENT *new_event;  
		   ...
		   new_event->operation = CLAIRE_WRITE;
       		   new_event->nbytes = event->nbytes;
		   new_event->startCycle = sim_cycle;
		   new_event->src1 = event->des1;
		   new_event->src2 = event->des2;
		   new_event->des1 = event->src1;
		   new_event->des2 = event->src2;
		   new_event->vp = event->vp; 
		   new_event->addr = event->addr;
		   new_event->processingDone = 0;
		   new_event->req_time = event->req_time;
		   new_event->cmd = Read;
		   new_event->mc_cmd = event->mc_cmd;
		   event->childCount ++;
		data_packet = new_event->nbytes / flitSize + (new_event->nbytes % flitSize > 0 ? 1 : 0);
	      	scheduleThroughNetwork(new_event, sim_cycle, data_packet, vc);
	      	dir_eventq_insert(new_event);
		break;			
	  }
        //-------------------------
	case CLAIRE_WRITE           :
		{
		  int data;
		  int data_int, cnt;
		int dst_id, src_id, nbytes, dst_context_id, src_context_id;
		md_addr_t dstaddr;//Processor ID * 0x00000100
		void *pdata = NULL;//data pointer
		flitSize = 4;
		dst_id = (event->des1)*8+event->des2;
		src_id = (event->src1)*8+event->src2;
		dst_context_id = dst_id - 8;
	        src_context_id = src_id - 8;
		...
		if(event->cmd == Read)
		  {
		    if(ON == event->mc_cmd)//read MC
		      {
			
			while(nbytes -- > 0)
			  {
			    data_int = EMint[(word_t)dstaddr];
			    MEM_WRITE(thecontexts[dst_context_id]->mem, (md_addr_t)pdata, int, data_int);//write the MC data to location of local pointer
			    pdata += sizeof(int);
			    dstaddr++;
			  }
			...
			while (nbytes -- >0)
			  {
			    data = MEM_READ(thecontexts[src_context_id]->mem, (md_addr_t)dstaddr, char);//read data from a source processor
			    MEM_WRITE(thecontexts[dst_context_id]->mem, (md_addr_t)pdata, char, data);
			    pdata ++;
			    dstaddr += sizeof(char);
			  }
		      }
		    //yangshuo-2013-7-5-data has been received, core continues to work
		    //for (cnt = 0; cnt < numcontexts; cnt++)
		      {
		      	cnt = dst_context_id;
		      	if ((quiesceAddrStruct[cnt].address == event->vp))
		      	  {
		  ...
		  if(ON == event->mc_cmd)//write to external memory
		    {
		      while(nbytes -- > 0)
			{
			  //data = MEM_READ_WORD(thecontexts[src_context_id]->mem, (md_addr_t)pdata);//read data from a source processor
			  data_int = MEM_READ(thecontexts[src_context_id]->mem, (md_addr_t)pdata, int);
			  EMint[(word_t)dstaddr] = data_int;

			  dstaddr++;
			  pdata += sizeof(int);
			}
		    }
		  else//write to processor
		    {  
		      while (nbytes -- >0)
			{
			  data = MEM_READ(thecontexts[src_context_id]->mem, (md_addr_t)pdata, char);//read data from a source processor
			  MEM_WRITE(thecontexts[dst_context_id]->mem, (md_addr_t)dstaddr, char, data);
			  pdata ++;
			  dstaddr += sizeof(char);
			}
		      // for (cnt = 0; cnt < numcontexts; cnt++)
		      {
			cnt = dst_context_id;
			if ((quiesceAddrStruct[cnt].address == event->addr))
			  {
			    thecontexts[cnt]->freeze = 0;
			    thecontexts[cnt]->running = 1;
			    quiesceAddrStruct[cnt].address = 1;
...
)
{
  ...
  /*2013-4-7*/
	...
	/*delete by yangshuo at 2013-5-27*/
	//	if (!strcmp (cp->name, dl1name))
	// 	{
	//if (cmd == Write)
	//{
