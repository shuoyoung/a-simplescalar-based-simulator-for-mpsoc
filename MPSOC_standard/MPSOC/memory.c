	...
	int nbytes, int threadID )			/* number of bytes to access */
{
	...
	{
		fprintf(stderr, "Memory access Problem %llx %d\n", (unsigned long long)addr, (int)nbytes);
		fflush(stderr);
		return md_fault_access;
	}
	if ((addr & (nbytes-1)) != 0)
	{
		printf( "Alignment Problem %llx %d\n", (unsigned long long)addr, (int)nbytes);
		fflush(stdout);
...
enum md_fault_type mem_access(struct mem_t *mem, enum mem_cmd cmd, md_addr_t addr, void *p,int nbytes, int threadID)
{
	...
	static struct wb_mem_ent *ent_wxh;
	context *current;
	...
	if (wxh_sdc == 1)
	{
			*((word_t *)&ent_wxh->data[1]) = *((word_t *)p);
			goto wxh_skip;
	}
