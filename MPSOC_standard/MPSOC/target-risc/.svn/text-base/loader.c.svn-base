...
/* program free segment base by yangshuo 2013-3-20 */
md_addr_t ld_free_base = 0;

/* program free size in bytes by yangshuo 2013-3-20 */
unsigned int ld_free_size = 0;
	     ...
	     int zero_bss_segs,int threadid)		/* zero uninit data segment? */
{
  ...
  md_addr_t sp, data_break = 0, null_ptr = 0, argv_addr, envp_addr;
  {
		...
		switch(phdr->p_type)
		{
			case PT_LOAD:
				...
				// ---text segment---//
				{
					...
					free(buffer);
					printf("ID %d text segment at %0x\n", current->id, phdr->p_vaddr);
				}
				else	
				// ---data segment---//
				{
					...
					mem_bcopy(mem_access,current->mem, Write, phdr->p_vaddr, buffer, phdr->p_filesz,current->id);
					free(buffer);
					printf("ID %d data segment at %0x\n", current->id, phdr->p_vaddr);
				}
				break;
			default:
				break;
    ...
    {/*DH:reset the shared mem from 0x0 to 0x00400000-----2013-4-7*/
	char *buf;
    buf=(char *)calloc(0x00400000,sizeof(byte_t));
	memset(buf, 0, 0x00400000*sizeof(byte_t));
	mem_bcopy(mem_access,current->mem, Write, 0x0, buf, 0x00400000,current->id);
	free(buf);
	printf("ID %d free segment at 0x00000000\n", current->id);
      fflush(stdout);
      current->ld_free_base = 0;
      }
    /* compute data segment size from data break point */
